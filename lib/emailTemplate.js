...
// 이상 항목이 여러 개여도 한 문장에 몰아넣지 않고, 항목마다 별도 섹션을 만든다.
function sentencesForSales(issues, standardPct) {
  return issues.map((i) => ({
    title: `${i.product} 판매 부진`,
    body: `9월 누적 판매진척률이 <strong>${i.pct}%</strong>로, 표준 진척률(${standardPct}%) 대비 5%p 이상 낮은 상태입니다.`,
    summary: `${i.product} 판매진척률이 표준 대비 5%p 이상 낮습니다.`,
    severity: "warn",
  }));
}

function sentencesForProduction(issues, standardPct) {
  return issues.map((i) => ({
    title: `${i.product_type} 생산 지연`,
    body: `${i.product_type} 생산진척률이 <strong>${i.pct}%</strong>로, 표준 진척률 대비 지연되고 있습니다.`,
    summary: `${i.product_type} 생산진척률이 표준 대비 지연되고 있습니다.`,
    severity: "warn",
  }));
}

function sentencesForCapa(issues) {
  return issues.map((i) => ({
    title: `${i.line} 가동률 저하`,
    body: `${i.line}의 CAPA 가동률이 <strong>${i.pct}%</strong>로, 기준선인 90%를 밑돌고 있습니다.`,
    summary: `${i.line} CAPA 가동률이 기준(90%)에 못 미칩니다.`,
    severity: "warn",
  }));
}
...
function buildIssueSections(briefing) {
  const sections = [
    ...sentencesForSales(briefing.issues.sales, briefing.standardPct),
    ...sentencesForProduction(briefing.issues.production, briefing.standardPct),
    ...sentencesForCapa(briefing.issues.capa),
...
function issueCardHtml(section, index) {
  ...
  return `
    <div style="border: 1px solid ${BORDER}; border-radius: 12px; padding: 18px 20px; background: ${CARD_BG}; margin-bottom: 14px;">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width: 100%; border-collapse: collapse; margin-bottom: 6px;">
        <tr>
          <td style="text-align: left; vertical-align: middle; font-weight: 700; font-size: 15px; color: ${color};">${index + 1}. ${section.title}</td>
          <td style="text-align: right; vertical-align: middle; white-space: nowrap; padding-left: 16px; width: 1%;">
            <span style="display: inline-block; padding: 3px 9px; border-radius: 6px; font-size: 11px; font-weight: 700; background: ${bg}; color: ${color};">${label}</span>
          </td>
        </tr>
      </table>
...
function buildEmailHtml(briefing, opts = {}) {
  const dashboardUrl = opts.dashboardUrl || "http://localhost:3000/dashboard.html";
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="${BG}" style="width: 100%; background: ${BG};">
  <tr>
    <td align="center" style="padding: 24px 12px;">
      <table role="presentation" width="640" cellpadding="0" cellspacing="0" border="0" bgcolor="${BG}" style="width: 100%; max-width: 640px; background: ${BG};">
        <tr>
          <td style="padding: 48px 44px; font-family: -apple-system, 'Apple SD Gothic Neo', system-ui, sans-serif; color: ${TEXT};" bgcolor="${BG}">

  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width: 100%; border-collapse: collapse; margin-bottom: 26px;">
    <tr>
      <td style="text-align: left; vertical-align: bottom;">
        ...
        <div style="font-size: 13px; color: ${TEXT_MUTED}; margin-top: 4px;">삼성디스플레이 / OLED</div>
      </td>
      <td style="text-align: right; vertical-align: bottom; white-space: nowrap; padding-left: 16px; width: 1%; font-size: 13px; color: ${TEXT_MUTED};">${briefing.referenceDate.replaceAll("-", ".")}</td>
    </tr>
  </table>
