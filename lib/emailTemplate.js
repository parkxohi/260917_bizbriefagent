...
function issueCardHtml(section, index) {
  return `
    <div style="border: 1px solid ${BORDER}; border-radius: 12px; padding: 18px 20px; background: ${CARD_BG}; margin-bottom: 14px;">
      <div style="font-weight: 700; font-size: 15px; color: ${ACCENT}; margin-bottom: 6px;">${index + 1}. ${section.title}</div>
...
function priorityRowHtml(section, index) {
  return `
    <div style="display: flex; gap: 12px; margin-bottom: 10px;">
      <span style="display: inline-block; width: 22px; height: 22px; line-height: 22px; text-align: center; border-radius: 999px; background: ${ACCENT}; color: ${BG}; font-size: 12px; font-weight: 700;">${index + 1}</span>
...
<div style="max-width: 640px; margin: 0 auto; background: ${BG}; padding: 48px 44px; font-family: -apple-system, 'Apple SD Gothic Neo', system-ui, sans-serif; color: ${TEXT};">
  ...
  <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 26px;">
    <div>
      <div style="font-family: Georgia, 'Noto Serif KR', serif; font-size: 20px; font-weight: 700; color: ${ACCENT};">Biz Briefing</div>
      <div style="font-size: 13px; color: ${TEXT_MUTED}; margin-top: 4px;">디스플레이 사업본부 · 경영진 대상</div>
    </div>
    <div style="font-size: 13px; color: ${TEXT_MUTED};">${briefing.referenceDate.replaceAll("-", ".")}</div>
  </div>

  <div style="font-size: 14px; color: ${TEXT_MUTED}; margin-bottom: 20px;">9월 1일~16일 누적 실적 기준 · 자동 분석 결과</div>
  ...
  <div style="margin-bottom: 20px;">
    <div style="font-weight: 700; font-size: 15px; color: ${TEXT}; margin-bottom: 12px;">지금 가장 먼저 볼 것</div>
    ${priorityRows}
  </div>` : ""}
  ...
  <div style="font-size: 12px; color: ${TEXT_FOOT}; text-align: center;">본 메일은 capstone 시연용으로 발송되었습니다 · Biz Briefing Agent</div>
